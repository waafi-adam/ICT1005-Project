...
} elseif (!empty($_POST['thumbnail_path'])) {
...
if ($success) {
    // Prepare the statement: 
    $stmt = $conn->prepare("UPDATE Product SET productName=?, productPrice=?, productCompany=?, productImage=?, productDescription=?, productImagePath=? WHERE productId=?");
    ...
    // Bind & execute the query statement: 
    $stmt->bind_param("sdsbssi", $dbfields['name'], $dbfields['price'], $dbfields['brand'], $thumbnail, $dbfields['description'], $thumbnail_path, $_POST['id']);
